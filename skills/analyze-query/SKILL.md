...
Spring @Transactional, JPA, QueryDSL 기반 코드의 트랜잭션 범위, 영속성 컨텍스트, 쿼리 실행 시점을 분석한다.
...
1. 분석 대상 유즈케이스를 특정한다.
   - 사용자가 지정한 API / 메서드 / 도메인 흐름
   - 지정되지 않은 경우, `@Transactional` 이 선언된 모든 Facade/Service를 탐색
2. 컨트롤러 → Facade → Service → Repository 전체 호출 흐름을 추적한다.
3. Checklist 1~3을 순서대로 수행하고 결과를 출력한다.
4. 문제가 발견되면 Checklist 4의 개선안을 선택지로 제시한다.

## Output Rules

- 코드에 근거하지 않은 추측은 하지 않는다.
- 현재 구조의 의도를 먼저 설명한 뒤, 문제점과 개선안을 제시한다.
- 정답이 아닌 선택지로 제시하며, trade-off를 함께 기술한다.
- 분석 결과는 유즈케이스 단위로 구분하여 출력한다.
