...
| 구분 | 헤더 | 적용 방식 |
|------|------|-----------|
| 대고객 | `X-Loopers-LoginId` + `X-Loopers-LoginPw` | `@MemberAuthenticated` 어노테이션 (`@Target(CLASS, FUNCTION)`) |
| 어드민 | `X-Loopers-Ldap: loopers.admin` | `@AdminAuthenticated` 어노테이션 (`@Target(CLASS, FUNCTION)`) |
...
application/{domain}/           → Facade, Service, Command, Info
domain/{domain}/                → Model(data class), Repository(interface), VO, Validator
infrastructure/{domain}/        → JpaModel(@Entity), RepositoryImpl, JpaRepository
```
- **모든 API는 Facade 경유**: Controller → Facade → Service. 단일 도메인이라도 반드시 Facade 경유.
- **Service 공유, Facade만 분리**: 고객/어드민 Facade가 동일한 Service를 사용.
- **고객/어드민 DTO 분리**: Controller 레벨에서 노출 필드 분리.
...
```
com.loopers
├── interfaces               # REST 엔드포인트, 요청 DTO 값 유효성 검증, 응답 DTO, 인증 설정
│   ├── api/                 # REST 컨트롤러, ApiSpec, DTO
│   └── config/              # 인증 어노테이션/Interceptor/ArgumentResolver, WebMvcConfig
├── application              # 트랜잭션 주체. Facade, Service, Command, Info DTO, AuthService (인증+캐시), ApplicationException, DomainExceptionTranslator
├── domain                   # Model(data class), Repository 인터페이스, VO, Validator, 포트 인터페이스(PasswordEncryptor), CoreException, ErrorType, PageQuery/PageResult
└── infrastructure           # JpaModel(@Entity), Repository 구현체, 포트 구현체(BcryptPasswordEncryptor), CursorUtils, 인프라 빈 설정 (CacheConfig)
```
...
- 단일 도메인 API는 Controller → Service 직접 호출 → 불필요한 Facade 경유 금지
- cross-domain API만 Facade 경유 → Service 간 직접 참조 금지
...
- 패키징 전략은 4개 레이어 패키지로 구성하고, 하위에 도메인 별로 패키징하는 형태로 작성합니다.
  - 예시
    > /interfaces/api (presentation 레이어 - REST 엔드포인트, PageResponse)
      /interfaces/config (presentation 레이어 - 인증 설정, WebMvcConfig)
      /application/.. (application 레이어 - Facade, Service, Command, Info DTO, AuthService)
      /domain/.. (domain 레이어 - Model(data class), Repository 인터페이스, VO, 포트 인터페이스, CoreException, ErrorType)
      /infrastructure/.. (infrastructure 레이어 - JpaModel(@Entity), Repository 구현체, 포트 구현체, CursorUtils, 인프라 빈 설정)
