...
│   │       ├── domain/          # 도메인 모델(Entity), Repository 인터페이스, VO, Validator
│   │       ├── infrastructure/  # Repository 구현체, JPA Repository, 인프라 빈 설정
│   │       └── support/         # 에러 핸들링 (ErrorType, CoreException), 유틸리티
...
application/{domain}/           → Facade (cross-domain만), Service, Info
domain/{domain}/                → Model(Entity), Repository(interface), VO, Validator, Command
infrastructure/{domain}/        → RepositoryImpl, JpaRepository
```
- **단일 도메인 API**: Controller → Service 직접 호출. Facade 경유하지 않음.
- **cross-domain API**: Controller → Facade → Service. 여러 도메인 조합이 필요한 경우에만 Facade 사용.
...
│   └── config/              # 인증 어노테이션/Interceptor/ArgumentResolver, WebMvcConfig
├── application              # 트랜잭션 주체. Facade (cross-domain 조합), Service (단일 도메인 로직), Info DTO
├── domain                   # Entity(Model), Repository 인터페이스, VO, Validator, Command
├── infrastructure           # Repository 구현체, JPA Repository, 인프라 빈 설정 (CacheConfig, PasswordEncoderConfig)
└── support                  # 에러 핸들링 (CoreException, ErrorType), 유틸리티 (CursorUtils)
```
...
- 패키징 전략은 4개 레이어 패키지 + cross-cutting 패키지로 구성하고, 하위에 도메인 별로 패키징하는 형태로 작성합니다.
  - 예시
    > /interfaces/api (presentation 레이어 - REST 엔드포인트)
      /interfaces/config (presentation 레이어 - 인증 설정, WebMvcConfig)
      /application/.. (application 레이어 - Facade, Service, Info DTO)
      /domain/.. (domain 레이어 - 도메인 객체 및 엔티티, Repository 인터페이스가 위치)
      /infrastructure/.. (infrastructure 레이어 - Repository 구현체, 인프라 빈 설정)
      /support/.. (cross-cutting - 에러 핸들링, 유틸리티)
