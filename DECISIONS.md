...
## 26. 좋아요 멱등 구현: 존재 확인 후 INSERT (DuplicateKeyException 미사용)

### 배경
Decision 17에서 좋아요 등록의 멱등성을 `DuplicateKeyException catch → 200 OK`로 설계했다. 그러나 구현 과정에서 `@Transactional` 내부에서 `DataIntegrityViolationException`이 발생하면 트랜잭션이 rollback-only로 마킹되어 `UnexpectedRollbackException`이 발생하는 문제를 발견했다.

### 선택지
| 선택지 | 설명 |
|--------|------|
| A. DuplicateKeyException catch | 설계 문서 원안. @Transactional 경계 이슈 |
| B. saveAndFlush() + Service 내부 catch | 즉시 flush로 예외를 Service에서 처리 |
| **C. 존재 확인 후 INSERT** | findByMemberIdAndProductId → null이면 save |

### 판단: C. 존재 확인 후 INSERT

### 근거
- **@Transactional 안전성**: `DataIntegrityViolationException`은 JPA flush/commit 시점에 발생하며, `@Transactional` 메서드 내부에서 catch해도 트랜잭션이 이미 rollback-only로 마킹된다. 이를 Facade에서 catch하면 `UnexpectedRollbackException`이 발생한다.
- **saveAndFlush() 미사용 원칙**: Decision 2에서 확립한 "Hibernate의 write-behind 최적화를 깨뜨리지 않는다"는 원칙과 일관.
- **UNIQUE Constraint 유지**: 존재 확인은 best-effort이며, 극히 드문 race condition에서는 UNIQUE Constraint가 최종 방어선 역할. 이 경우 예외가 발생하지만, 실제 동시 좋아요 빈도를 고려하면 무시 가능한 수준.
- **코드 단순성**: `if (existing != null) return` 한 줄로 멱등 처리 완료. try-catch보다 의도가 명확.

### 트레이드오프
- SELECT 1회 추가 (정상 케이스에서도 조회 발생) → UNIQUE 인덱스 기반 조회이므로 O(1). 좋아요 API의 TPS 특성상 무시 가능.
- check-then-act 패턴으로 이론적 race condition 존재 → UNIQUE Constraint가 최종 방어. 실패 시 500 에러이나 확률적으로 무시 가능.

### Decision 17 수정사항
- 시퀀스 다이어그램의 "DuplicateKeyException catch → 200 OK" 설명을 "존재 확인 → 이미 있으면 200 OK"로 변경.

---

## 27. 좋아요 목록 API: userId 경로 변수 제거

### 배경
설계 문서에서 좋아요 목록 조회 API를 `GET /api/v1/users/{userId}/likes`로 정의하고, BR-L3에서 `userId ≠ 인증된 memberId`이면 403 Forbidden을 반환하도록 설계했다. 그러나 구현 과정에서 이 설계의 실용성을 재검토했다.

### 선택지
| 선택지 | 설명 |
|--------|------|
| A. /api/v1/users/{userId}/likes | 설계 원안. Facade에서 userId ≠ memberId 검증 |
| **B. /api/v1/likes** | 인증된 사용자 본인의 좋아요만 조회. userId 불필요 |

### 판단: B. `/api/v1/likes`

### 근거
- **불필요한 파라미터 제거**: 인증된 사용자는 항상 본인의 좋아요만 조회한다. userId를 경로에 포함하면 본인 것만 조회 가능한데 굳이 userId를 전달하는 불필요한 설계.
- **FORBIDDEN 분기 제거**: userId ≠ memberId 검증 자체가 불필요해짐. 코드 단순화.
- **REST 설계 관점**: "나의 좋아요 목록"은 인증 컨텍스트에서 자연스럽게 결정되므로, 경로에 userId를 포함할 이유가 없다. `/api/v1/members/me` 패턴과 일관.

### 트레이드오프
- 어드민이 특정 사용자의 좋아요를 조회하는 API가 필요할 경우 별도 엔드포인트 추가 필요 → 현재 스코프에서 해당 요구사항 없음.

### 영향받는 설계 문서
- 01-requirements.md: Like API URL 변경, BR-L3 수정
- 02-sequence-diagrams.md: 좋아요 목록 조회 흐름 변경
- 03-class-diagram.md: LikeV1Controller, LikeFacade 시그니처 변경

---

## 28. 대고객 상품 목록: 커서 기반 페이징

### 배경
설계 문서에서 대고객 상품 목록을 offset 기반 페이징(`page=0&size=20`)으로 정의했으나, 대규모 데이터에서의 성능과 사용자 경험을 고려하여 커서 기반 페이징으로 변경했다.

### 선택지
| 선택지 | 설명 |
|--------|------|
| A. Offset 페이징 | `LIMIT offset, size`. 대규모에서 deep page 성능 저하 |
| **B. 커서 기반 페이징** | WHERE 조건 기반. Base64 인코딩 커서. 일정한 성능 |

### 판단: B. 커서 기반 페이징 (대고객 상품 목록만)

### 근거
- **일정한 성능**: offset 페이징은 deep page에서 `OFFSET N` 행을 스킵해야 하므로 성능 저하. 커서는 WHERE 조건으로 직접 접근하므로 페이지 깊이와 무관하게 일정한 성능.
- **적용 범위 한정**: 대고객 상품 목록만 커서 적용. 어드민 목록, 주문 목록은 기존 offset 유지. 대고객 목록이 가장 트래픽이 높고 데이터가 많으므로 최적화 대상.
- **정렬별 커서 구조**: `latest`(id DESC), `price_asc`(price ASC, id DESC), `likes_desc`(like_count DESC, id DESC). 모든 정렬에서 id를 tie-breaker로 사용하여 결정론적 순서 보장.

### 트레이드오프
- 커서 기반은 "N페이지로 바로 이동" 불가 → 무한 스크롤 UI에 적합. 대고객 상품 목록의 UX와 부합.
- 커서 인코딩/디코딩 로직 추가 → CursorUtils로 캡슐화하여 복잡도 관리.
- 어드민은 offset 유지 → 관리 UI에서는 페이지 번호 기반 탐색이 더 적합.
