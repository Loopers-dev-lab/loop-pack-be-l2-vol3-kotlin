#!/usr/bin/env bash
# commerce-api PreToolUse Guard — 보호 브랜치 직접 커밋 차단
...
# G1: 보호 브랜치(main, develop, master)에서 직접 커밋 차단
case "$INPUT" in
  *git*commit*)
      ...
      cat <<EOF
{
  "hookSpecificOutput": {
    ...
    "permissionDecisionReason": "${CURRENT_BRANCH} 브랜치에서는 직접 커밋할 수 없습니다. 작업 브랜치(feature/*, fix/*, refactor/*)를 먼저 생성하세요."
  }
}
EOF
      exit 0
