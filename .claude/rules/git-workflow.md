...
`.claude/`, `docs/` 등 설정/문서 파일을 수정할 때도 **main에서 직접 수정하지 마라.**

1. **파일을 수정하기 전에** 작업 브랜치를 먼저 생성하라. 수정 후에 브랜치를 만드는 것이 아니라, 브랜치를 만든 뒤 수정을 시작하라.
2. 단, 긴급 수정(hotfix)은 예외로 할 수 있으나 사용자의 명시적 승인이 필요하다.
...
| 변경 대상 | 커밋 위치 |
|-----------|-----------|
| `apps/commerce-api/` 내부 | 프로젝트 루트에서 커밋 |
| `modules/`, `supports/` 내부 | 프로젝트 루트에서 커밋 |
| `.claude/`, `docs/`, 루트 설정 파일 | 프로젝트 루트에서 커밋 |

### 커밋 메시지 접두사

`feat:` | `refactor:` | `fix:` | `test:` | `docs:` | `chore:`

**스킬 우선 사용:** 커밋은 `/ship` 스킬을 사용하라. git 명령어를 직접 실행하지 마라.
...
2. **main이 아닌 브랜치에 있다면**:
   - `gh pr view --json state,mergedAt`로 해당 브랜치의 PR 상태를 확인
