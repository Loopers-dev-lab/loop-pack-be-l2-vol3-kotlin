...
커밋을 분석한 결과, 아래 표현 방식이 적합한 경우 **적극적으로** 사용한다:

- **AS-IS / TO-BE 테이블**: 기존 동작과 변경 후 동작을 대비할 수 있을 때
- **Step별 구현 설명**: 커밋이 단계별로 나뉘어 있거나, 변경이 논리적 단계로 구분될 때. 각 Step에 Before/After 코드, Mermaid 다이어그램, 고민한 점을 포함할 수 있다
- **설정값 근거 테이블** (`⚙️ 설정값 근거`): 설정 변경(Kafka, Redis, DB 등)이 있을 때만 포함. `값 | 기본값 | 근거` 형식
- **Mermaid 다이어그램**: 전체 아키텍처(flowchart), 주요 흐름(sequence) 등 시각화가 도움될 때 적극 사용
- **코드 링크**: PR diff 파일 링크(`#diff-...`)로 리뷰어가 본문에서 바로 코드를 확인할 수 있게 연결

이 표현 방식들은 강제가 아니라, 커밋 분석 결과에 따라 가장 효과적인 방식을 선택한다.
...
(PR 생성 전 사용자에게 리뷰 포인트를 질문하여 작성한다. 각 포인트는 Mermaid/비교 테이블/코드 링크를 활용하여 리뷰어가 맥락을 빠르게 파악할 수 있게 구성한다.)
```

## PR 생성

``0. 리뷰 포인트 수집: 본문 초안을 사용자에게 보여준 뒤 "리뷰 포인트로 넣고 싶은 내용이 있으면 알려주세요."라고 질문한다.
   - 사용자가 포인트를 제공하면 `🔍 리뷰 포인트` 섹션에 반영 (Mermaid/비교 테이블/코드 링크 활용)
   - "없음"이면 섹션 자체를 제거
