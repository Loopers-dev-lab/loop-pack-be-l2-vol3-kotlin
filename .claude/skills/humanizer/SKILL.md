...
**수정 전:**
> React는 현재 가장 널리 사용되는 프론트엔드 라이브러리라고 할 수 있습니다.

**수정 후:**
> React는 현재 가장 많이 쓰이는 프론트엔드 라이브러리다.
...
**수정 전:**
> 먼저 데이터를 수집합니다. 이를 통해 패턴을 분석할 수 있습니다. 이를 바탕으로 모델을 학습시킵니다.
...
**감지 표현:** "~의 중요성은 아무리 강조해도 지나치지 않습니다", "~은/는 매우 중요합니다", "~에서 핵심적인 역할을 합니다", "~에 있어서 필수적입니다", "~의 중요성이 날로 커지고 있습니다"

**문제:** 왜 중요한지 구체적으로 말하지 않고 중요하다고만 반복한다.

**수정 전:**
> 코드 리뷰의 중요성은 아무리 강조해도 지나치지 않습니다.

**수정 후:**
> 코드 리뷰를 거친 코드는 프로덕션 버그가 적다. 리뷰어가 작성자가 놓친 엣지 케이스를 잡아내기 때문이다.
...
**문제:** AI가 구조를 잡기 위해 스스로 질문을 던지고 답하는 패턴.
...
**감지 표현:** "결론적으로", "요약하자면", "마무리하며", "지금까지 ~에 대해 알아보았습니다", "앞으로 ~이/가 기대됩니다", "~의 발전이 기대됩니다"
...
**감지 표현:** "또한" (과용), "한편", "더불어", "아울러", "나아가", "특히" (과용), "무엇보다"
...
**문제:** AI가 새로운 것을 소개할 때 "기존"과의 대비를 억지로 만든다.
...
### E1. 중요성 과장 [P1]
**감지:** testament, pivotal, crucial, evolving landscape, indelible mark

### E3. ~ing 접미 분석 [P1]
**감지:** highlighting..., underscoring..., showcasing..., fostering...

### E4. 홍보성 언어 [P1]
**감지:** nestled, groundbreaking, vibrant, robust, seamless, leverage, cutting-edge

### E7. AI 빈출 어휘 [P1]
**감지:** Additionally, delve, tapestry, interplay, intricate, nuanced, paradigm

### E8. 계사 회피 [P2]
**감지:** serves as, stands as, marks a, represents a

### E9. 부정 병렬 구조 [P2]
**감지:** Not only...but..., It's not just about...

### E10. False Ranges [P2]
**감지:** from X to Y (의미 없는 범위)

### E11. Em dash 과용 [P2]
한 단락에 em dash 1개 이하. 나머지는 쉼표나 괄호로 교체.

### E12. 볼드체 과용 / 인라인 헤더 목록 [P2]
`- **Header:** Description` 패턴 반복.

### E13. 대화형 잔류물 [P1]
**감지:** Great question!, I'd be happy to, Let's dive in, Here's the thing
...
### E16. Curly 따옴표 [P3]
모두 straight quotes로 교체.

### E17. 제목 Title Case [P3]
Sentence case로 교체.

### E18. 불필요한 요약 반복 [P1]
**감지:** In summary, To summarize, As we've seen

### E19. 과도한 구조화 [P2]
산문이 더 자연스러운 곳에서 번호 목록 남발.
...
### C1. 동의어 순환 [P2]
같은 대상을 다른 단어로 계속 바꿔 부름.

### C2. 지식 한계 면책 [P1]
"정확한 정보는 확인이 필요합니다" → 전부 삭제.

### C3. 긍정적 결론 공식 [P1]
"밝은 미래가 기대됩니다" → 구체적 다음 단계로 교체하거나 삭제.

### C4. 이모지 장식 [P1]
사용자가 명시적으로 요청하지 않으면 전부 제거.

### C5. 균일한 문단 길이 [P2]
문단 길이가 기계적으로 균일하면 리듬감을 만든다.

### C6. 강제 3단 구조 [P2]
500자 미만의 글에 "결론적으로"는 거의 항상 불필요.
