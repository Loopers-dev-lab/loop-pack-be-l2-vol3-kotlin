---
name: analysis-sequence-diagram
description:
  제공된 요구사항을 분석하고, 개발자와의 질문/대답을 통해 애매한 요구사항을 명확히 하여 정리합니다.
  모든 정리가 끝나면, 시퀀스 다이어그램, Mermaid 문법으로 작성한다.
...
### 8 요청에 대한 처리 흐름️은 아래를 기본으로 하는데 필요시 추가나 변경한다
- controller -> facade(필요시) -> service -> repository
  - facade가 필요한 경우 facade는 비즈니스 로직을 갖지 않고 호출 대리자로서의 역할로 서비스들의 비즈니스를 조합하여 달성한다
...
- 경로: /design
  - 없으면 디렉토리를 생성한다
- 생성하려는 파일의 이름이 존재하는 경우 v1, v2 처럼 뒤에 버전을 붙여서 생성한다
