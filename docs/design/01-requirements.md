...
현재는 '주문됨(ORDERED)' 하나만 존재한다. 이후 결제 기능이 추가되면 '결제 완료(PAID)', '취소(CANCELLED)' 등으로 확장할 예정이다. COMPLETED가 아닌 ORDERED로 명명한 이유는, COMPLETED는 "배송 완료"까지 연상시키기 때문이다. 지금부터 상태 필드를 두는 이유는, 나중에 상태가 늘어나도 기존 구조를 바꾸지 않아도 되기 때문이다.
...
**결정: 필수 파라미터, 최대 조회 기간 1년(365일).**

**왜 이렇게 결정했는가:** 기간을 반드시 지정해야 한다. 기본값을 두면 클라이언트가 "아무 생각 없이" 전체 조회를 할 수 있고, 이는 시스템 부하로 이어진다. 조회 기간 상한을 1년으로 제한하여, 대량 데이터 스캔으로 인한 성능 저하를 원천 방지한다. 1년을 초과하는 조회가 필요한 경우, 여러 번 나누어 요청하도록 안내한다.
...
**API 파라미터**: `sort=latest`(기본값), `sort=price_asc`, `sort=popularity`. 클라이언트가 정렬 기준을 지정하지 않으면 `latest`가 적용된다.

---

#### 정책 27. 페이지네이션 기본값과 상한

**결정: page=0, size=20 (기본값), size 최대 100.**

**왜 이렇게 결정했는가:** 모든 페이지네이션이 필요한 기능에서 동일한 기본값을 사용하여 일관성을 유지한다. size 상한을 100으로 제한하여, 의도치 않은 대량 조회로 인한 시스템 부하를 방지한다. 100을 초과하는 요청은 거부한다(400 Bad Request).
...
#### 정책 32. 상품의 "삭제"와 "판매 중지"는 동일한 메커니즘을 사용한다

**어떤 상황인가:** 상품이 비활성화되는 경로는 두 가지이다.
1. 어드민이 상품을 직접 삭제하는 경우 (상품 삭제)
2. 브랜드 삭제로 인해 연쇄적으로 비활성화되는 경우 (판매 중지)

두 경우의 원인은 다르지만, 결과(고객에게 비노출, 주문/좋아요 불가)는 동일하다. 이 두 상태를 구분할 필요가 있는가?

| 관점 | 별도 상태 필드 도입 (`status`) | 동일 메커니즘 (`deleted_at`) |
|------|------------------------------|---------------------------|
| 복구 유연성 | "브랜드 복구 시 연쇄 비활성 상품도 자동 복구" 가능 | 어떤 원인으로 비활성화됐는지 구분 불가 |
| 구현 복잡도 | status 필드 추가, 조회 조건에 분기 로직 필요 | `deleted_at IS NULL` 단일 조건으로 충분 |
| 현재 요구사항 | 브랜드/상품 복구 기능이 없음 | 충분함 |

**결정: 동일 메커니즘 (`deleted_at`).**

**왜 이렇게 결정했는가:** 현재 브랜드나 상품의 복구 기능은 범위에 없다. 존재하지 않는 요구사항을 위해 상태 필드를 추가하면, 모든 조회 쿼리에 분기 로직이 생겨 불필요한 복잡도가 발생한다. YAGNI 원칙을 따른다. 비활성화의 **결과**가 동일하므로, 원인을 구분할 실익이 없다.

**향후:** 복구 기능이 필요해지면, `deactivation_reason` 컬럼 추가 또는 이벤트 로그로 비활성화 원인을 추적할 수 있다. `deleted_at` 기반 설계를 바꿀 필요는 없으며, 컬럼 추가만으로 대응 가능하다.
...
| 26 | 상품 정렬 기본값 | 최신순 (sort=latest, price_asc, popularity) | ✅ |
| 27 | 페이지네이션 기본값과 상한 | page=0, size=20, 최대 100 | ✅ |
...
| 32 | 판매 중지와 삭제의 통합 메커니즘 | deleted_at 단일 필드 (YAGNI) | ✅ |
...
| 주문됨 | Ordered | 재고 확인과 차감이 모두 끝나고 거래가 기록된 상태 |
