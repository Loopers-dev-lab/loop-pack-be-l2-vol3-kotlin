...
1) Customer는 브랜드 상세 정보를 조회한다. (FR-001)
2) Customer는 브랜드 필터/정렬/페이지 조건으로 상품 목록을 조회한다. (FR-002)
3) Customer는 상품 상세 정보를 조회한다. (FR-003)
4) Customer는 상품 좋아요를 등록한다. (FR-014)
5) Customer는 상품 좋아요를 취소한다. (FR-015)
6) Customer는 본인 좋아요 상품 목록을 조회한다. (FR-016)
7) Customer는 여러 상품을 한 번에 주문한다. (FR-017)
8) Customer는 기간 조건으로 본인 주문 목록을 조회한다. (FR-018)
9) Customer는 본인 단일 주문 상세를 조회한다. (FR-019)
10) Admin은 브랜드 목록을 조회한다. (FR-004)
11) Admin은 브랜드 상세를 조회한다. (FR-005)
12) Admin은 브랜드를 등록한다. (FR-006)
13) Admin은 브랜드 정보를 수정한다. (FR-007)
14) Admin은 브랜드를 삭제하고, 연관 상품 삭제 처리 결과를 확인한다. (FR-008)
15) Admin은 상품 목록을 조회한다. (FR-009)
16) Admin은 상품 상세를 조회한다. (FR-010)
17) Admin은 기존 브랜드에 속한 상품을 등록한다. (FR-011)
18) Admin은 브랜드 변경 없이 상품 정보를 수정한다. (FR-012)
19) Admin은 상품을 삭제한다. (FR-013)
20) Admin은 주문 목록을 조회한다. (FR-020)
21) Admin은 단일 주문 상세를 조회한다. (FR-021)
