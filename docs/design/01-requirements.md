...
|------|------|------|
| 쿠폰 발급 | 회원 | 쿠폰을 발급받는다 |
| 내 쿠폰 조회 | 회원 | 발급받은 쿠폰 목록을 조회한다 |
| 쿠폰 등록 | 어드민 | 새 쿠폰을 등록한다 |
| 쿠폰 수정 | 어드민 | 쿠폰 정보를 수정한다 |
| 쿠폰 삭제 | 어드민 | 쿠폰을 삭제한다 |
...
| BR-O08 | 주문 시 쿠폰을 적용할 수 있다 (선택 사항) |
| BR-O09 | 쿠폰 적용 시 할인 정보(원래 금액, 할인 금액)가 주문에 스냅샷으로 저장된다 |
| BR-O10 | 주문 취소 시 사용된 쿠폰은 AVAILABLE 상태로 복원된다 |

### 쿠폰

| 규칙 | 설명 |
|------|------|
| BR-CP01 | 쿠폰 할인 유형은 정액(FIXED)과 정률(RATE)이 있다 |
| BR-CP02 | 정률 할인의 할인값은 1~100 사이여야 한다 |
| BR-CP03 | 쿠폰에는 최소 주문 금액 조건이 있을 수 있다 |
| BR-CP04 | 쿠폰에는 최대 발급 수량 제한이 있을 수 있다 (null이면 무제한) |
| BR-CP05 | 한 사용자는 같은 쿠폰을 1회만 발급받을 수 있다 |
...
| 상태 | 의미 | 설명 |
|------|------|------|
| AVAILABLE | 사용 가능 | 발급되었으나 아직 사용되지 않은 상태 |
| USED | 사용됨 | 주문에 적용되어 사용된 상태 |
| EXPIRED | 만료됨 | 유효기간이 지난 상태 |

```
현재:
  AVAILABLE → USED (주문 시 사용)
  USED → AVAILABLE (주문 취소 시 복원)
```
...
**Main Flow**
1. 회원은 쿠폰 발급을 요청한다
2. 시스템은 쿠폰 존재 여부와 삭제/만료 여부를 확인한다
3. 시스템은 중복 발급 여부를 확인한다 (BR-CP05)
4. 시스템은 발급 가능 수량을 확인한다 (BR-CP04)
5. 시스템은 쿠폰 정보를 스냅샷하여 UserCoupon을 생성한다
6. 시스템은 쿠폰의 issuedCount를 원자적으로 증가시킨다
7. 회원에게 발급 성공을 응답한다

**Alternate Flow**
- A1: 쿠폰이 존재하지 않거나 삭제됨 → 404
- A2: 만료된 쿠폰 → 400
- A3: 이미 발급받은 쿠폰 → 409 (BR-CP05)
- A4: 발급 수량 초과 → 409 (BR-CP04)

### 쿠폰 적용 주문 생성

**Main Flow**
1. 회원은 상품과 수량, 쿠폰 ID(선택)를 포함하여 주문을 요청한다
2. 시스템은 각 상품의 재고를 검증하고 차감한다 (기존 주문 생성과 동일)
3. 쿠폰이 지정된 경우:
   - 시스템은 UserCoupon을 비관적 락으로 조회한다 (SELECT FOR UPDATE)
   - 시스템은 소유자, 상태(AVAILABLE), 만료 여부, 최소 주문 금액을 검증한다
   - 시스템은 할인 금액을 계산한다 (FIXED: 정액, RATE: 정률)
   - 시스템은 UserCoupon 상태를 USED로 변경한다
4. 시스템은 주문을 저장한다 (originalAmount, discountAmount, totalAmount 포함)
5. 회원에게 주문 생성 성공을 응답한다

**Alternate Flow**
- A1: 쿠폰이 본인 것이 아님 → 403
- A2: 쿠폰이 이미 사용됨(USED) → 400
- A3: 쿠폰이 만료됨 → 400
- A4: 최소 주문 금액 미달 → 400
- A5: 동시 사용으로 상태 전이 실패 → 비관적 락에 의해 순차 처리됨
...
| 만료된 쿠폰 발급/사용 | 400 BAD_REQUEST | BR-CP06 |
| 이미 발급받은 쿠폰 재발급 | 409 CONFLICT | BR-CP05 |
