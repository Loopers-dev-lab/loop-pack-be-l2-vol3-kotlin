...
sequenceDiagram
    ...
    alt @MemberAuthenticated (고객 인증)
        I->>AS: authenticate(loginId, rawPassword)
        AS->>Cache: get(loginId)
        alt Cache HIT + SHA256 match
            Cache-->>AS: CachedAuth
            AS-->>I: AuthResult
        else Cache MISS or SHA256 mismatch
            AS->>MS: authenticate(loginId, rawPassword)
            MS-->>AS: MemberModel
            AS->>Cache: put(loginId, CachedAuth)
            AS-->>I: AuthResult
        end
...
인증, 캐싱, 에러 핸들링 등 횡단 관심사(cross-cutting concerns)의 구성 요소와 연결 관계를 파악한다. DIP 원칙에 따라 각 구성 요소가 올바른 레이어에 위치하는지를 검증한다.

```mermaid
graph TB
    subgraph Interfaces["Interfaces Layer"]
        subgraph Auth["인증 (Presentation)"]
            MAI["MemberAuthenticationInterceptor<br/>AuthService 호출"]
    ...
    subgraph DomainLayer["Domain Layer"]
        ...
        subgraph DomainPort["포트 인터페이스 (Domain)"]
            PE["PasswordEncryptor<br/>encode() · matches()"]
        end
    end
...
| Domain | `PasswordEncryptor` | Interface (포트): encode(), matches() — 인프라 기술 추상화 |
...
| Infrastructure | `BcryptPasswordEncryptor` | PasswordEncryptor 구현체 (BCryptPasswordEncoder 래핑) |
...
| Domain | `ProductRepository` | Interface: save, findById, findByIdWithLock, findActiveProducts (CursorResult 반환) |
| Domain | `ProductStatus` | Enum: ACTIVE, DELETED |
| Domain | `ProductSearchCondition` | 커서 페이징 쿼리 조건 (cursor: String?) |
...
| Domain/Common | `PageQuery` | 도메인 페이징 요청 (page, size) |
| Domain/Common | `PageResult<T>` | 도메인 페이징 결과 (content, totalElements, totalPages) |
| Domain/Common | `CursorResult<T>` | 도메인 커서 페이징 결과 (content, nextCursor, hasNext) |
| Domain/VO | `Email` | `@JvmInline value class`: 이메일 형식 검증 |
| Domain/Error | `ErrorType` | 에러 코드 Enum (BAD_REQUEST, NOT_FOUND, UNAUTHORIZED, FORBIDDEN, CONFLICT). HttpStatus 없음 (순수 도메인) |
...
| Infrastructure | `CursorUtils` | 커서 인코딩/디코딩 (Base64+JSON) |
...
| 패턴 | 적용 | 관련 Decision |
|------|------|---------------|
| DIP (의존성 역전) | Domain → Repository/PasswordEncryptor(IF), Infrastructure → Impl. Presentation→Domain 0건, Application→Infrastructure 0건 | D30 |
| Facade 분리 | 모든 API는 Facade 경유. 고객/어드민 Facade 분리, Service 공유. Facade만 `@Transactional` | D8 |
| Domain Model 분리 | Domain Model은 순수 data class. JPA Entity(JpaModel)는 Infrastructure 레이어에 격리 | D30 |
| 포트 패턴 | PasswordEncryptor(Domain IF) ← BcryptPasswordEncryptor(Infrastructure Impl) | D30 |
| AuthService | Interceptor → AuthService(Application) 단일 의존. 인증+캐시 로직을 Application으로 격리 | D30 |
| PageResponse | Presentation 전용 페이징 DTO. Controller가 Domain PageResult에 직접 의존하지 않음 | D30 |
| 예외 계층 변환 | CoreException(Domain) → DomainExceptionTranslator(@Aspect) → ApplicationException → ApiControllerAdvice | D30 |
