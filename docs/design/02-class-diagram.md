...
%% Relationships - Value Object 사용
    ...
    User --> UserName
    User --> Email
    Product --> Money: regularPrice, sellingPrice
...
- **판매 단위의 정보 관리**: 상품명, 정상가/판매가, 소속 브랜드 정보를 보유한다
- **노출 제어**: 활성/비활성 상태를 통해 Customer 조회 가능 여부를 결정한다

#### True Invariant

- `name`, `regularPrice`, `sellingPrice` 필수
- `sellingPrice ≤ regularPrice` (판매가는 정상가를 초과할 수 없다)
...
- **Product**: 카탈로그 정보 (name, regularPrice, sellingPrice, brandId)의 정합성
...
| 정상가/판매가 분리 (할인 표시)        | 가격 이원화 정책 (향후 프로모션 확장)           |
...
classDiagram
    ...
    OrderItem --> Quantity
    OrderSnapshot --> Money: regularPrice, sellingPrice
    Order --> IdempotencyKey
    Order --> OrderStatus
...
- **OrderSnapshot (Value Object)**: 주문 시점의 불변 정보를 보존하는 구조 VO다
    - productId, productName, brandId, brandName, regularPrice(Money), sellingPrice(Money), thumbnailUrl을 포함한다
...
- **사용처**: Product.regularPrice, Product.sellingPrice, OrderSnapshot.regularPrice, OrderSnapshot.sellingPrice
...
- **구성**: productId, productName, brandId, brandName, regularPrice(Money), sellingPrice(Money), thumbnailUrl
...
- Product의 불변식: 카탈로그 정보 (name, regularPrice, sellingPrice, brandId)
...
| **5.3 스냅샷 보존**         | OrderSnapshot VO                 | 주문 생성 시점의 productName, brandName, regularPrice, sellingPrice, thumbnailUrl을 OrderSnapshot에 보존한다. 이후 Product/Brand 변경 시 영향 없다 |
