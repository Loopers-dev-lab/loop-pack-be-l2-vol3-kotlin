...
그래서 timeout이 나면 FAILED로 전이하지 않고 PENDING을 유지한 뒤, 복구 스케줄러가 orderId로 PG에 직접 물어보는 방식을 택했습니다.

PG에 거래가 있으면 txKey를 받아서 결과를 반영하고, 거래가 없으면 그때 FAILED로 전이시킵니다.
이 방식이면 유저 돈이 빠졌는데 주문이 안 잡히는 상황이 생기지 않습니다.

다만 REQUESTED 상태에서 유저가 결과를 모르는 시간이 최대 5분 정도 발생할 수 있는데, 이건 프론트에서 polling이나 push로 풀어야 하는 영역이라 서버 코드만으로는 줄이기 어렵습니다.
