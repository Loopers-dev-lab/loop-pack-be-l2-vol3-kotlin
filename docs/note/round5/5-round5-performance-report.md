...
### 실측 EXPLAIN ANALYZE actual time 비교 (10만건, 워밍업 1회 + 10회 평균)

| 쿼리              | AS-IS (ms) | TO-BE (ms) | 개선율      |
|-----------------|-----------|-----------|----------|
| 브랜드 필터 + 좋아요 정렬 | 42.920    | 1.453     | **29.5x**   |
| 전체 좋아요 정렬      | 47.300    | 0.053     | **899.6x**  |
| 최신순 전체 조회      | 48.950    | 0.034     | **1423.0x** |
| 가격순 전체 조회      | 53.520    | 0.042     | **1283.5x** |

- TestContainers 환경에서는 버퍼 풀에 전체 데이터가 적재되므로 절대 수치보다 **상대 개선율**이 의미 있음
- 브랜드 필터 쿼리(29.5x)가 전체 조회(900~1400x)보다 개선율이 낮은 이유: 인덱스 순서로 스캔하며 `ref_brand_id` 조건에 맞는 row를 찾을 때까지 더 많은 row를 건너뛰어야 함
...
- **인덱스 EXPLAIN**: `rows` 값이 TestContainers 환경에서 통계가 부정확하여 `1`로 표시됨. 실제 운영 DB에서는 인덱스 유무에 따라 rows 차이가 극적으로 발생한다. 핵심은
  `type=ALL → ref` 전환이다.
- **비정규화**: 동시성 테스트는 성능 비교가 아닌 정합성 증명이다. EXPLAIN 비교는 이론적 분석이며 실측은 아니다.
- **캐시**: TestContainers 환경은 네트워크 레이턴시가 거의 없어 DB 직접 조회와 캐시 히트의 차이가 작게 나온다. 운영 환경에서는 DB I/O가 훨씬 비싸므로 개선율이 더 클 것으로 예상된다.
  초반 측정값이 높은 것은 JIT/커넥션풀 워밍업 영향이다.
