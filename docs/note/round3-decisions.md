...
| 선택지 | 구성                                                         | 장점                            | 단점                                                       |
|-----|------------------------------------------------------------|-------------------------------|----------------------------------------------------------|
| A   | `public constructor` + `guard()` (Product/UserPoint 패턴 통일) | 코드베이스 일관성                     | Order는 생성 시 복합 조립(OrderItem + totalPrice 계산)이 필요해 단순 생성 패턴에 맞지 않음 |
