...
| **중**  | [8. 기타 설계 결정](#8-기타-설계-결정)                                                 | Order 생성 패턴, 조회 메서드 분리, 도메인 경계 |
| **중**  | [9. 동시성/멱등성](#9-동시성멱등성)                                                    | 재고 전략, 2-Phase 주문              |
...
- **Domain Service 사용 시점**: 여러 Entity 간 원자적 얽힘이 있는 경우에만
- **cross-domain 오케스트레이션**: UseCase가 여러 도메인의 Repository를 직접 주입받아 처리 (예: `PlaceOrderUseCase`가 ProductRepository + OrderRepository 조합)
...
회원가입 시 `RegisterUserUseCase`가 UserRepository를 직접 주입받아 User를 생성한다.

### 8.2 Order 생성 패턴 — private constructor + create()
