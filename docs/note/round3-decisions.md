...
- 재고 차감은 `Product.decreaseStock()`, 가격 검증은 `Product.guard()`에서 인라인 검증, 이름 검증은 `Brand.guard()`에서 인라인 검증
...
**문제:** Order는 다른 엔티티(Product, UserPoint)와 달리 `guard()` 메서드를 사용하지 않고, `private constructor` +
`companion object { create() }` 패턴을 사용한다. 이 차이를 명확히 기록하지 않으면 "다른 엔티티와 패턴을 통일해야 한다"거나 "Facade에서 OrderItem 조립과 totalPrice
...
- **Product, UserPoint와의 차이**: 이들은 생성 후에도 `update()`, `charge()`, `use()` 등으로 상태가 변한다. `guard()`가 `@PrePersist`/
  `@PreUpdate`마다 재검증하는 의미가 있다
- **Order의 특성**: `Order.create()`는 `userId`와 `totalPrice`를 전달받아 Order 객체만 생성한다. totalPrice 계산, OrderItem 생성, 저장은
  OrderService가 담당한다. Order와 OrderItem은 JPA 연관관계 매핑(@OneToMany/@ManyToOne) 없이 `refOrderId` FK로 연결되며, 별도
  `OrderItemRepository`로 관리한다. 생성 이후에는 OrderStatus를 제외하면 전부 불변이므로
  재검증할 필드가 없다
- **private constructor**: `create()`를 우회하여 검증 없이 객체를 만드는 걸 컴파일 타임에 차단한다
- 이 패턴 선택 기준은 CLAUDE.md의 "Entity 생성 패턴 선택 기준"에도 반영하였다
- **OrderItem도 동일한 패턴을 따른다**: `private constructor` + `create()`, 생성 후 불변이므로 `guard()` 불필요. `init` 블록에서
  `quantity >= 1` 직접 검증으로 생성 시점 검증만 수행
...
| 컨텍스트    | 조회 메서드                                   | 반환 범위                   |
|---------|------------------------------------------|-------------------------|
| 대고객 조회  | `getActiveBrand()`, `getActiveProduct()` | 활성 엔티티만 (삭제/HIDDEN 필터링) |
