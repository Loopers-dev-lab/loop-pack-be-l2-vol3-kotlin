...
export function checkRankings() {
    ...
    if (res.status === 200) {
        ...
        console.log('========== 랭킹 결과 (10x) ==========');
        rankings.forEach((r) => {
            let label = '';
            if (r.productId == 1) label = ' <-- 정상 유저 50명';
            if (r.productId == 2) label = ' <-- 단일IP 봇 100회';
            if (r.productId == 3) label = ' <-- UA없는 봇 50개';
            console.log(`  ${r.rank}위: 상품${r.productId} (score: ${r.score.toFixed(4)})${label}`);
        });
        if (rankings.length === 0) console.log('  (랭킹 데이터 없음)');
        console.log('======================================');
    }
}
