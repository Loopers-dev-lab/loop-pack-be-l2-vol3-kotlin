...
interface ProductJpaRepository : JpaRepository<ProductEntity, Long> {
    ...
    @Query("SELECT p FROM ProductEntity p WHERE p.deletedAt IS NULL ORDER BY p.price ASC")
    fun findAllActivePriceAsc(): List<ProductEntity>

    @Query("SELECT p FROM ProductEntity p WHERE p.brandId = :brandId AND p.deletedAt IS NULL ORDER BY p.price ASC")
