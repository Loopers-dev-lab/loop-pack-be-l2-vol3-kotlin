...
- **Domain Service 사용 시점**: 여러 Entity 간 원자적 얽힘이 있거나 복잡한 정책 검증이 필요한 경우에만 Domain Service에 위임한다.
...
**금지:** Domain Service에 `@Transactional`을 사용하지 않는다. 트랜잭션 경계는 반드시 UseCase(Application 계층)에서 관리한다.
