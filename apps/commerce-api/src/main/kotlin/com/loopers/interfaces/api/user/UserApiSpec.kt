...
interface UserApiSpec {
    ...
    fun getMe(userInfo: AuthenticatedUserInfo): ApiResponse<UserDto.MeResponse>

    @Operation(
        summary = "비밀번호 변경",
        description = "현재 비밀번호를 확인 후 새 비밀번호로 변경합니다.",
    )
    fun changePassword(userInfo: AuthenticatedUserInfo, request: UserDto.ChangePasswordRequest): ApiResponse<Unit>
}
