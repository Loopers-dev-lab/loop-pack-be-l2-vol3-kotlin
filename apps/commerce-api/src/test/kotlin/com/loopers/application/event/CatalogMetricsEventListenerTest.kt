...
class CatalogMetricsEventListenerTest {
    ...
    inner class HandleCatalogEvent {
        ...
        @Test
        @DisplayName("LikeAdded 이벤트 → CatalogOutbox에 LIKE_ADDED로 기록된다")
        fun handleLikeAdded() {
            ...
            assertThat(outboxList[0].eventType).isEqualTo("LIKE_ADDED")
            assertThat(outboxList[0].productId).isEqualTo(1L)
            assertThat(outboxList[0].userId).isEqualTo(2L)
        }

        @Test
        @DisplayName("LikeRemoved 이벤트 → CatalogOutbox에 LIKE_REMOVED로 기록된다")
        fun handleLikeRemoved() {
            ...
            val outboxList = catalogOutboxRepository.findAllUnpublished()
            assertThat(outboxList).hasSize(1)
            assertThat(outboxList[0].eventType).isEqualTo("LIKE_REMOVED")
        }

        @Test
        @DisplayName("ProductViewed 이벤트 → CatalogOutbox에 PRODUCT_VIEWED로 기록된다")
