...
@DisplayName("OrderDetailUseCase")
class OrderDetailUseCaseTest {

    private val orderRepository: OrderRepository = mock()
    private val useCase = OrderDetailUseCase(orderRepository)
    ...
    @Nested
    @DisplayName("본인 주문 조회 시 OrderResult.Detail을 반환한다")
    inner class WhenOwnOrder {

        @Test
        @DisplayName("정상 조회 → Detail(orderId, userId, status, items, totalAmount, createdAt)")
        fun getDetail_success() {
            // arrange
            given(orderRepository.findByIdAndUserId(100L, 1L)).willReturn(order())

            // act
            val result = useCase.getDetail(orderId = 100L, userId = 1L)
    ...
    inner class WhenOrderNotFound {
        ...
        fun getDetail_notFound() {
            // arrange
            given(orderRepository.findByIdAndUserId(999L, 1L)).willReturn(null)

            // act
            val exception = assertThrows<CoreException> {
                useCase.getDetail(orderId = 999L, userId = 1L)
            }
    ...
    @Nested
    @DisplayName("타인의 주문 조회 시 ORDER_NOT_FOUND 예외를 던진다")
    inner class WhenOtherUserOrder {

        @Test
        @DisplayName("userId 불일치 → 예외")
        fun getDetail_otherUser() {
            // arrange
            given(orderRepository.findByIdAndUserId(100L, 999L)).willReturn(null)

            // act
            val exception = assertThrows<CoreException> {
                useCase.getDetail(orderId = 100L, userId = 999L)
            }
