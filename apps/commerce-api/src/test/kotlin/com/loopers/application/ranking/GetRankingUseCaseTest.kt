...
import com.loopers.domain.ranking.fixture.FakeMonthlyRankingRepository
import com.loopers.domain.ranking.fixture.FakeRankingRepository
import com.loopers.domain.ranking.fixture.FakeWeeklyRankingRepository
...
class GetRankingUseCaseTest {
    ...
    @Nested
    inner class GetDailyRankingPage {

        @Test
        fun `일간 랭킹 페이지를 조회하면 상품 정보가 포함된 결과를 반환한다`() {
            ...
            val result = getRankingUseCase.getRankingPage(DATE, RankingPeriod.DAILY, 1, 20)
        ...
        fun `페이지네이션이 정상 동작한다`() {
            ...
            val page1 = getRankingUseCase.getRankingPage(DATE, RankingPeriod.DAILY, 1, 2)
            val page2 = getRankingUseCase.getRankingPage(DATE, RankingPeriod.DAILY, 2, 2)
        ...
        fun `다른 날짜의 랭킹은 서로 독립적이다`() {
            ...
            val yesterday = getRankingUseCase.getRankingPage("20260408", RankingPeriod.DAILY, 1, 20)
            val today = getRankingUseCase.getRankingPage("20260409", RankingPeriod.DAILY, 1, 20)
    ...
    @Nested
    inner class GetWeeklyRankingPage {

        @Test
        fun `주간 랭킹을 조회하면 해당 주의 MV 데이터를 반환한다`() {
            val brandId = brandRepository.save(createBrand())
            val productId1 = productRepository.save(createProduct(brandId, "상품A"))
            val productId2 = productRepository.save(createProduct(brandId, "상품B"))

            val monday = LocalDate.of(2026, 4, 13)
            weeklyRankingRepository.addScore(monday, productId1, 250.0)
            weeklyRankingRepository.addScore(monday, productId2, 180.0)

            val result = getRankingUseCase.getRankingPage("20260417", RankingPeriod.WEEKLY, 1, 20)
    ...
    @Nested
    inner class GetMonthlyRankingPage {

        @Test
        fun `월간 랭킹을 조회하면 해당 월의 MV 데이터를 반환한다`() {
            val brandId = brandRepository.save(createBrand())
            val productId1 = productRepository.save(createProduct(brandId, "상품A"))
            val productId2 = productRepository.save(createProduct(brandId, "상품B"))

            val firstOfMonth = LocalDate.of(2026, 4, 1)
            monthlyRankingRepository.addScore(firstOfMonth, productId1, 500.0)
            monthlyRankingRepository.addScore(firstOfMonth, productId2, 350.0)

            val result = getRankingUseCase.getRankingPage("20260417", RankingPeriod.MONTHLY, 1, 20)
