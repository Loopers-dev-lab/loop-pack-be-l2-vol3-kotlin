...
@DisplayName("OrderListUseCase")
class OrderListUseCaseTest {

    private val orderRepository: OrderRepository = mock()
    private val useCase = OrderListUseCase(orderRepository)
    ...
    @Nested
    @DisplayName("ListItem에 orderSummary와 totalAmount가 정상 포함된다")
    inner class WhenListItemContent {

        @Test
        @DisplayName("단일 상품 → 상품명만 표시")
        fun getList_singleItemSummary() {
            // arrange
            val today = LocalDate.now()
            val defaultFrom = today.minusMonths(1).atStartOfDay(ZoneId.systemDefault())
            val defaultTo = today.plusDays(1).atStartOfDay(ZoneId.systemDefault())
