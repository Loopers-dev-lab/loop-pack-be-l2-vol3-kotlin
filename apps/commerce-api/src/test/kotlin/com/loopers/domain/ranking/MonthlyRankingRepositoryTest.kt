...
class MonthlyRankingRepositoryTest {

    private lateinit var repository: FakeMonthlyRankingRepository
    ...
    inner class FindAllByPeriodKey {
        ...
        fun `rank 오름차순 100건 반환`() {
            ...
            // Act
            val result = repository.findAllByPeriodKey("2026-04")
        ...
        fun `존재하지 않는 periodKey 빈 목록 반환`() {
            // Act
            val result = repository.findAllByPeriodKey("2026-99")
        ...
        @Test
        @DisplayName("100건 이하면 저장된 수만큼 반환한다")
        ...
        fun `다른 periodKey 격리`() {
            // Arrange
            repository.addEntry(entry(rank = 1, productId = 1L, periodKey = "2026-04"))
            repository.addEntry(entry(rank = 1, productId = 2L, periodKey = "2026-03"))
