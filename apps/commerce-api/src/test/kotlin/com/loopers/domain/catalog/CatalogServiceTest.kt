...
class CatalogServiceTest {
    ...
    inner class CreateBrand {
        ...
        fun createBrand_withValidName_savesAndReturnsBrand() {
            // arrange
            val command = CatalogCommand.CreateBrand(name = "나이키")
    ...
    inner class UpdateBrand {
        ...
        fun updateBrand_nonExistent_throwsNotFound() {
            // act
            val exception = assertThrows<CoreException> {
                catalogService.updateBrand(999L, CatalogCommand.UpdateBrand(name = "아디다스"))
            }
    ...
    inner class GetBrand {
        ...
        fun getBrands_returnsPagedResults() {
            // arrange
            catalogService.createBrand(CatalogCommand.CreateBrand(name = "나이키"))
            catalogService.createBrand(CatalogCommand.CreateBrand(name = "아디다스"))
            catalogService.createBrand(CatalogCommand.CreateBrand(name = "뉴발란스"))
    ...
    inner class GetProducts {
        ...
        fun getProducts_excludesDeletedAndHidden() {
            ...
            catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "상품1", price = BigDecimal("10000"), stock = 10),
            )
            val hidden = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "상품2", price = BigDecimal("20000"), stock = 10),
            )
            catalogService.updateProduct(
                hidden.id,
                CatalogCommand.UpdateProduct(name = null, price = null, stock = null, status = Product.ProductStatus.HIDDEN),
            ...
            val deleted = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "상품3", price = BigDecimal("30000"), stock = 10),
            )
        ...
        fun getProducts_filtersByBrandId() {
            // arrange
            val brand1 = catalogService.createBrand(CatalogCommand.CreateBrand(name = "나이키"))
            val brand2 = catalogService.createBrand(CatalogCommand.CreateBrand(name = "아디다스"))
            catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand1.id, name = "나이키 상품", price = BigDecimal("10000"), stock = 10),
            )
            catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand2.id, name = "아디다스 상품", price = BigDecimal("20000"), stock = 10),
            )
        ...
        fun getProducts_sortsByPriceAsc() {
            ...
            val brand = catalogService.createBrand(CatalogCommand.CreateBrand(name = "나이키"))
            catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "비싼상품", price = BigDecimal("50000"), stock = 10),
            )
            catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "싼상품", price = BigDecimal("10000"), stock = 10),
            )
        ...
        fun getProducts_sortsByLikesDesc() {
            ...
            val product1 = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "인기없는상품", price = BigDecimal("10000"), stock = 10),
            )
            val product2 = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "인기상품", price = BigDecimal("20000"), stock = 10),
            )
    ...
    inner class InternalProductQueries {
        ...
        fun getActiveProductsByIds_excludesDeletedAndHidden() {
            ...
            val active = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "활성상품", price = BigDecimal("10000"), stock = 10),
            )
            val hidden = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "숨김상품", price = BigDecimal("20000"), stock = 10),
            )
            catalogService.updateProduct(
                hidden.id,
                CatalogCommand.UpdateProduct(name = null, price = null, stock = null, status = Product.ProductStatus.HIDDEN),
            ...
            val deleted = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "삭제상품", price = BigDecimal("30000"), stock = 10),
            )
            catalogService.deleteProduct(deleted.id)
    ...
    inner class DecreaseStocks {
        ...
        fun decreaseStocks_success() {
            ...
            val product2 = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "상품2", price = BigDecimal("20000"), stock = 5),
            )
        ...
        fun decreaseStocks_insufficientStock_throwsException() {
            ...
            val brand = catalogService.createBrand(CatalogCommand.CreateBrand(name = "나이키"))
            val product = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "상품1", price = BigDecimal("10000"), stock = 5),
            )
    ...
    inner class LikeCount {
        ...
        fun increaseLikeCount_incrementsByOne() {
            ...
            val brand = catalogService.createBrand(CatalogCommand.CreateBrand(name = "나이키"))
            val product = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "에어맥스 90", price = BigDecimal("129000"), stock = 100),
            )
    ...
    inner class AdminQueries {
        ...
        fun getAdminProducts_includesDeleted() {
            ...
            val deleted = catalogService.createProduct(
                CatalogCommand.CreateProduct(brandId = brand.id, name = "삭제상품", price = BigDecimal("20000"), stock = 10),
            )
            catalogService.deleteProduct(deleted.id)
