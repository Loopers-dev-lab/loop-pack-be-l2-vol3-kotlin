...
class ProductStockUnitTest {
    ...
    fun `ProductStock init allows quantity of 0`() {
        val stock = createStock(quantity = 0)
    ...
    @Test
    fun `isSoldOut returns true when quantity is 0`() {
        val stock = createStock(quantity = 0)
        assertThat(stock.isSoldOut).isTrue()
    }

    @Test
    fun `isSoldOut returns false when quantity is greater than 0`() {
        val stock = createStock(quantity = 1)
    ...
    @Test
    fun `validate() throws CoreException(BAD_REQUEST) when qty exceeds stock`() {
    ...
    @Test
    fun `validate() throws CoreException(BAD_REQUEST) when qty is zero`() {
    ...
    @Test
    fun `validate() throws CoreException(BAD_REQUEST) when qty is negative`() {
        val stock = createStock(quantity = 10)
        assertThrows<CoreException> {
            stock.validate(-1)
        }.also {
    ...
    @Test
    fun `decrement() should reduce quantity by qty`() {
        val stock = createStock(quantity = 10)
        stock.decrement(3)
    ...
    @Test
    fun `decrement() should succeed when qty equals quantity (boundary)`() {
        val stock = createStock(quantity = 5)
        stock.decrement(5)
        assertThat(stock.quantity).isEqualTo(0)
    }
    ...
    @Test
    fun `decrement() throws CoreException(BAD_REQUEST) when qty exceeds quantity`() {
        val stock = createStock(quantity = 3)
        assertThrows<CoreException> {
            stock.decrement(4)
        }.also {
    ...
    @Test
    fun `decrement() throws CoreException(BAD_REQUEST) when qty is zero`() {
        val stock = createStock(quantity = 10)
        assertThrows<CoreException> {
            stock.decrement(0)
        }.also {
    ...
    @Test
    fun `decrement() throws CoreException(BAD_REQUEST) when qty is negative`() {
        val stock = createStock(quantity = 10)
        assertThrows<CoreException> {
            stock.decrement(-1)
        }.also {
    ...
    @Test
    fun `update() should set quantity to new value`() {
        val stock = createStock(quantity = 10)
        stock.update(50)
