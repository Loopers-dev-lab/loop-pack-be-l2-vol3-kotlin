package com.loopers.batch.job.weeklyrank

import com.loopers.batch.job.weeklyrank.step.WeeklyRankDeleteTasklet
...
class WeeklyRankJobConfig(
    ...
    companion object {
        ...
        private const val DELETE_STEP_NAME = "weeklyRankDeleteStep"
        private const val CHUNK_STEP_NAME = "weeklyRankChunkStep"
        ...
        private const val WRITER_SQL = """
            INSERT INTO mv_product_rank_weekly (product_id, score, created_at, updated_at)
            VALUES (?, ?, NOW(), NOW())
        """
    }

    @Bean(JOB_NAME)
    fun weeklyRankJob(): Job {
        return JobBuilder(JOB_NAME, jobRepository)
            .incrementer(RunIdIncrementer())
            .start(weeklyRankDeleteStep())
            .next(weeklyRankChunkStep())
    ...
    @Bean
    fun weeklyRankReader(): JdbcCursorItemReader<RankRow> {
    ...
    @Bean
    fun weeklyRankWriter(): JdbcBatchItemWriter<RankRow> {
