 ...
 * 월간 랭킹 Job.
 *
 * Step 1 (Tasklet): 해당 (year, month)의 기존 MV 행을 hard delete. 멱등성 보장.
 ...
 *   JpaItemWriter가 `mv_product_rank_monthly`에 INSERT. rank 산출을 SQL에 위임해 restart/병렬 실행에 안전하다.
 */
...
class MonthlyRankJobConfig(
    ...
    private val monthlyRankProcessor: MonthlyRankProcessor,
) {
    companion object {
        const val JOB_NAME = "monthlyRankJob"
        const val DELETE_STEP_NAME = "monthlyRankDeleteStep"
        const val CHUNK_STEP_NAME = "monthlyRankStep"
    ...
    fun monthlyRankStep(
        ...
        writer: JpaItemWriter<MonthlyProductRankEntity>,
    ): Step =
        StepBuilder(CHUNK_STEP_NAME, jobRepository)
            .chunk<AggregatedProductMetric, MonthlyProductRankEntity>(CHUNK_SIZE, transactionManager)
            .reader(reader)
    ...
    fun monthlyRankReader(
        ...
        val endDate = baseDate.withDayOfMonth(baseDate.lengthOfMonth())
    ...
    @Bean
    fun monthlyRankWriter(
        entityManagerFactory: EntityManagerFactory,
    ): JpaItemWriter<MonthlyProductRankEntity> =
        JpaItemWriterBuilder<MonthlyProductRankEntity>()
            .entityManagerFactory(entityManagerFactory)
            .build()
}
