...
class MonthlyRankingAggregationJobTest @Autowired constructor(
    private val jobLauncherTestUtils: JobLauncherTestUtils,
    @param:Qualifier(MonthlyRankingAggregationJobConfig.JOB_NAME) private val job: Job,
    private val jdbcTemplate: JdbcTemplate,
    private val databaseCleanUp: DatabaseCleanUp,
) {
    private val periodKey = "202604"
    private val startDate = "2026-04-01"
    private val endDate = "2026-04-30"
    ...
    @DisplayName("monthlyRankingAggregationJob 실행 시 mv_product_rank_monthly에 5개 row가 rank_value 1~5 순으로 적재된다")
    @Test
    fun shouldAggregateMonthlyRanking() {
        ...
        val ranks = jdbcTemplate.queryForList(
            "SELECT rank_value FROM mv_product_rank_monthly WHERE period_key = ? ORDER BY rank_value",
            Int::class.java,
            periodKey,
        )
        val scores = jdbcTemplate.queryForList(
            "SELECT score FROM mv_product_rank_monthly WHERE period_key = ? ORDER BY rank_value",
            Double::class.java,
            periodKey,
        )
    ...
    @DisplayName("monthlyRankingAggregationJob을 두 번 실행해도 결과가 동일하다 (멱등성)")
    @Test
    fun shouldBeIdempotentOnRerun() {
        // arrange
        jobLauncherTestUtils.job = job
        ...
        // act: 1차 실행
        val firstExecution = jobLauncherTestUtils.launchJob(params1)
        ...
        // act: 2차 실행
        val secondExecution = jobLauncherTestUtils.launchJob(params2)

        // assert: 두 실행 모두 COMPLETED, MV row 개수 동일
